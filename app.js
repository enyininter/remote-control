...
// ── Detección: ¿PC (tiene PAIR_CODE) o celular? ───────────
const isPC = window.PAIR_CODE && window.PAIR_CODE.length === 6;

if (isPC) {
  // ══════════════════════════════════════════════════════
  //  MODO PC — muestra el código, entra directo a la app
  // ══════════════════════════════════════════════════════
  showPCMode();

  const code = window.PAIR_CODE;
  ...
  // El PC ya tiene SERVER_IP en window (inyectado por config.js local)
  showApp();
  initApp();

} else {
  // ══════════════════════════════════════════════════════
  //  MODO CELULAR — ingresa el código
  // ══════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════
//  INIT APP — se llama cuando ya tenemos IP y puertos
// ══════════════════════════════════════════════════════════
function initApp() {
  ...
  function startStream() {
    ...
    streamWs.onopen = () => {
      ...
      fpsT = setInterval(() => { streamSt.textContent=`● ${frameN} fps`; streamSt.className=frameN>0?"stream-on":"stream-buf"; frameN=0; }, 1000);
    };
    streamWs.onmessage = async (e) => {
      try { const bmp=await createImageBitmap(new Blob([e.data],{type:"image/jpeg"})); ctx.drawImage(bmp,0,0,canvas.width,canvas.height); bmp.close(); frameN++; } catch(_){}
    };
    streamWs.onclose = () => { btnStream.textContent="▶ Iniciar"; btnStream.classList.remove("active"); cursorDot.style.display="none"; streamSt.textContent="● Sin señal"; streamSt.className="stream-off"; clearInterval(fpsT); };
    streamWs.onerror = () => streamWs.close();
  }
  function stopStream() { streamWs?.close(); streamWs=null; }
  btnStream.addEventListener("click", () => { (streamWs&&streamWs.readyState===WebSocket.OPEN)?stopStream():startStream(); });
  ...
  screenWrap.addEventListener("touchstart",e=>{e.preventDefault();if(e.touches.length===2){st.dist=d2(e.touches[0],e.touches[1]);st.mid=m2(e.touches[0],e.touches[1]);}},{passive:false});
  screenWrap.addEventListener("touchmove",e=>{e.preventDefault();if(e.touches.length!==2)return;const[t0,t1]=[e.touches[0],e.touches[1]],nd=d2(t0,t1),nm=m2(t0,t1),r=screenWrap.getBoundingClientRect();if(st.dist){const f=nd/st.dist,ax=nm.x-r.left-r.width/2,ay=nm.y-r.top-r.height/2,pz=zoom;zoom=Math.max(ZOOM_MIN,Math.min(ZOOM_MAX,zoom*f));panX=ax-(ax-panX)*(zoom/pz);panY=ay-(ay-panY)*(zoom/pz);}if(st.mid){panX+=nm.x-st.mid.x;panY+=nm.y-st.mid.y;}st.dist=nd;st.mid=nm;applyTransform();},{passive:false});
  screenWrap.addEventListener("touchend",e=>{e.preventDefault();if(e.touches.length<2){st.dist=null;st.mid=null;}if(e.touches.length===0)applyTransform();},{passive:false});

  // ── Joystick ────────────────────────────────────────────
  const joyBase=document.getElementById("joystickBase"),joyKnob=document.getElementById("joystickKnob");
  const JR=36,JD=4,JS=18,JI=16;
  let joyActive=false,joyVec={x:0,y:0},joyTimer=null,joyTouchId=null;
  const joyCenter=()=>{const r=joyBase.getBoundingClientRect();return{x:r.left+r.width/2,y:r.top+r.height/2};};
  function setKnob(ox,oy){const d=Math.hypot(ox,oy),cl=d>JR?JR/d:1;joyKnob.style.transform=`translate(calc(-50% + ${ox*cl}px),calc(-50% + ${oy*cl}px))`;const n=Math.min(d,JR)/JR;joyVec=d<JD?{x:0,y:0}:{x:(ox/d)*n,y:(oy/d)*n};}
  function resetKnob(){joyKnob.style.transform="translate(-50%,-50%)";joyVec={x:0,y:0};joyBase.classList.remove("active");}
  function joyTick(){if(!joyActive||(joyVec.x===0&&joyVec.y===0))return;const sp=JS*Math.pow(Math.hypot(joyVec.x,joyVec.y),1.5),dx=Math.round(joyVec.x*sp),dy=Math.round(joyVec.y*sp);if(dx||dy)send({action:"move",dx,dy});}
  joyBase.addEventListener("touchstart",e=>{e.preventDefault();if(joyTouchId!==null)return;const t=e.changedTouches[0];joyTouchId=t.identifier;joyActive=true;joyBase.classList.add("active");const c=joyCenter();setKnob(t.clientX-c.x,t.clientY-c.y);joyTimer=setInterval(joyTick,JI);vibrate(15);},{passive:false});
  joyBase.addEventListener("touchmove",e=>{e.preventDefault();const t=[...e.changedTouches].find(x=>x.identifier===joyTouchId);if(!t)return;const c=joyCenter();setKnob(t.clientX-c.x,t.clientY-c.y);},{passive:false});
  function joyEnd(e){e.preventDefault();const t=[...e.changedTouches].find(x=>x.identifier===joyTouchId);if(!t)return;joyActive=false;joyTouchId=null;clearInterval(joyTimer);resetKnob();}
